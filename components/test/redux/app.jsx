...
var ShowOneStockInfo = React.createClass({
  ...
  componentDidMount: function() {
      ...
      success : function(result){
        if (this.isMounted()) {
          ...
          _.forEach(result.showapi_res_body.list, function(value, key){
            var one = [value.date, value.open_price, value.close_price, value.min_price, value.max_price];
            historyData.push(one);
          });
    ...
    var myChart = echarts.init(chart, 'roma');
    
    // 数据意义：开盘(open)，收盘(close)，最低(lowest)，最高(highest)
    var data0 = splitData(this.state.historyData); // todo
...
ReactDOM.render(
  <ShowOneStockInfo source="http://apis.baidu.com/showapi_open_bus/stock_his/sh_stock_his" begin="2015-09-01" end="2015-09-28" code="600004" />,
